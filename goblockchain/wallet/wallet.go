...
func NewWallet() *Wallet {
	...
	digest2 := h2.Sum(nil)
	//3. perform RIPEMD160 hashing on the result of SHA-256 (20 bytes)
	h3 := ripemd160.New()
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	...
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	//5. perform SHA-256 hashing on the extended RIPEMD160 result
	h5 := sha256.New()
	h5.Write(vd4)
	digest5 := h5.Sum(nil)
	//6. perform SHA-256 hashing on the result of the previous SHA-256 hash
	h6 := sha256.New()
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
...
func (w *Wallet) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		BlockchainAddressKey string `json:"blockchain_address"`
	}{
		PrivateKey:           w.PrivateKeyStr(),
		PublicKey:            w.PublicKeyStr(),
		BlockchainAddressKey: w.BlockchainAddress(),
	})
}
...
func (t *Transaction) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		Recipiet string  `json:"recipient_blockchain_address"`
		Value    float32 `json:"value"`
	}{
		Sender:   t.senderBlockchainAddress,
		Recipiet: t.recipientBlockchainAddress,
		Value:    t.value,
	})
}
